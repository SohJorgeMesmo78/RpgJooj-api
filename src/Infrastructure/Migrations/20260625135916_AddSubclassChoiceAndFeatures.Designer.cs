// <auto-generated />
using System.Collections.Generic;
using Infrastructure.Persistence;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;

#nullable disable

namespace Infrastructure.Migrations
{
    [DbContext(typeof(AppDbContext))]
    [Migration("20260625135916_AddSubclassChoiceAndFeatures")]
    partial class AddSubclassChoiceAndFeatures
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "10.0.9")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("Domain.Entities.AcaoPersonagem", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("AcertoTooltip")
                        .HasMaxLength(150)
                        .HasColumnType("character varying(150)");

                    b.Property<string>("Alcance")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<string>("BonusAcerto")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<string>("Dano")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<string>("DanoTooltip")
                        .HasMaxLength(150)
                        .HasColumnType("character varying(150)");

                    b.Property<string>("Descricao")
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<int>("IdPersonagem")
                        .HasColumnType("integer");

                    b.Property<string>("Nome")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<string>("TipoAcao")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<string>("TipoDano")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.HasKey("Id");

                    b.HasIndex("IdPersonagem");

                    b.ToTable("AcoesPersonagens", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1,
                            AcertoTooltip = "[FOR](FOR) + [PROF](Proficiência)",
                            Alcance = "1.5m / 5ft",
                            BonusAcerto = "+1",
                            Dano = "1",
                            DanoTooltip = "1 (Base) + [FOR](FOR)",
                            Descricao = "Ataque desarmado básico.",
                            IdPersonagem = 1,
                            Nome = "Soco",
                            TipoAcao = "Ação",
                            TipoDano = "Impacto"
                        });
                });

            modelBuilder.Entity("Domain.Entities.CaracteristicaClasse", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("Descricao")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<int>("IdClasse")
                        .HasColumnType("integer");

                    b.Property<int>("Nivel")
                        .HasColumnType("integer");

                    b.Property<string>("Nome")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("character varying(150)");

                    b.HasKey("Id");

                    b.HasIndex("IdClasse");

                    b.ToTable("CaracteristicasClasses", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Descricao = "Seu patrono concede a você características no 1º nível e novamente no 6º, 10º e 14º nível.",
                            IdClasse = 1,
                            Nivel = 1,
                            Nome = "Patrono Transcendental"
                        },
                        new
                        {
                            Id = 2,
                            Descricao = "Sua pesquisa arcana e a magia infundida em você por seu patrono concedem a você a capacidade de conjurar magias.",
                            IdClasse = 1,
                            Nivel = 1,
                            Nome = "Magia de Pacto"
                        },
                        new
                        {
                            Id = 3,
                            Descricao = "Em suas investigações sobre o conhecimento oculto, você descobriu invocações místicas, fragmentos de conhecimento proibido que infundem você com uma habilidade mágica duradoura.",
                            IdClasse = 1,
                            Nivel = 2,
                            Nome = "Invocações Místicas"
                        },
                        new
                        {
                            Id = 4,
                            Descricao = "No 3º nível, seu patrono transcendental concede a você um favor por seus serviços leais. Você ganha uma das características: Pacto da Corrente, Pacto da Lâmina ou Pacto do Tomo.",
                            IdClasse = 1,
                            Nivel = 3,
                            Nome = "Dádiva do Pacto"
                        },
                        new
                        {
                            Id = 5,
                            Descricao = "Você pode aumentar um valor de atributo de sua escolha em 2, ou dois valores de atributo de sua escolha em 1. Como normal, você não pode aumentar um valor de atributo acima de 20 usando essa característica.",
                            IdClasse = 1,
                            Nivel = 4,
                            Nome = "Incremento no Valor de Habilidade"
                        },
                        new
                        {
                            Id = 6,
                            Descricao = "Seu Patrono Transcendental concede a você uma característica no 6º nível baseado na sua escolha de Patrono.",
                            IdClasse = 1,
                            Nivel = 6,
                            Nome = "Característica de Patrono Transcendental"
                        },
                        new
                        {
                            Id = 7,
                            Descricao = "Você pode aumentar um valor de atributo de sua escolha em 2, ou dois valores de atributo de sua escolha em 1. Como normal, você não pode aumentar um valor de atributo acima de 20 usando essa característica.",
                            IdClasse = 1,
                            Nivel = 8,
                            Nome = "Incremento no Valor de Habilidade"
                        },
                        new
                        {
                            Id = 8,
                            Descricao = "Seu Patrono Transcendental concede a você uma característica no 10º nível baseado na sua escolha de Patrono.",
                            IdClasse = 1,
                            Nivel = 10,
                            Nome = "Característica de Patrono Transcendental"
                        },
                        new
                        {
                            Id = 9,
                            Descricao = "Seu patrono confere a você um segredo mágico chamado de Arcana Mística. Escolha uma magia de 6º nível da lista de magias de bruxo como sua arcana mística. Você pode conjurar essa magia uma vez sem gastar um espaço de magia. Você deve terminar um descanso longo antes de poder fazer isso novamente.",
                            IdClasse = 1,
                            Nivel = 11,
                            Nome = "Arcana Mística (6° nível)"
                        },
                        new
                        {
                            Id = 10,
                            Descricao = "Você pode aumentar um valor de atributo de sua escolha em 2, ou dois valores de atributo de sua escolha em 1. Como normal, você não pode aumentar um valor de atributo acima de 20 usando essa característica.",
                            IdClasse = 1,
                            Nivel = 12,
                            Nome = "Incremento no Valor de Habilidade"
                        },
                        new
                        {
                            Id = 11,
                            Descricao = "Seu patrono confere a você um segredo mágico chamado de Arcana Mística. Escolha uma magia de 7º nível da lista de magias de bruxo como sua arcana mística. Você pode conjurar essa magia uma vez sem gastar um espaço de magia. Você deve terminar um descanso longo antes de poder fazer isso novamente.",
                            IdClasse = 1,
                            Nivel = 13,
                            Nome = "Arcana Mística (7° nível)"
                        },
                        new
                        {
                            Id = 12,
                            Descricao = "Seu Patrono Transcendental concede a você uma característica no 14º nível baseado na sua escolha de Patrono.",
                            IdClasse = 1,
                            Nivel = 14,
                            Nome = "Característica de Patrono Transcendental"
                        },
                        new
                        {
                            Id = 13,
                            Descricao = "Seu patrono confere a você um segredo mágico chamado de Arcana Mística. Escolha uma magia de 8º nível da lista de magias de bruxo como sua arcana mística. Você pode conjurar essa magia uma vez sem gastar um espaço de magia. Você deve terminar um descanso longo antes de poder fazer isso novamente.",
                            IdClasse = 1,
                            Nivel = 15,
                            Nome = "Arcana Mística (8° nível)"
                        },
                        new
                        {
                            Id = 14,
                            Descricao = "Você pode aumentar um valor de atributo de sua escolha em 2, ou dois valores de atributo de sua escolha em 1. Como normal, você não pode aumentar um valor de atributo acima de 20 usando essa característica.",
                            IdClasse = 1,
                            Nivel = 16,
                            Nome = "Incremento no Valor de Habilidade"
                        },
                        new
                        {
                            Id = 15,
                            Descricao = "Seu patrono confere a você um segredo mágico chamado de Arcana Mística. Escolha uma magia de 9º nível da lista de magias de bruxo como sua arcana mística. Você pode conjurar essa magia uma vez sem gastar um espaço de magia. Você deve terminar um descanso longo antes de poder fazer isso novamente.",
                            IdClasse = 1,
                            Nivel = 17,
                            Nome = "Arcana Mística (9° nível)"
                        },
                        new
                        {
                            Id = 16,
                            Descricao = "Você pode aumentar um valor de atributo de sua escolha em 2, ou dois valores de atributo de sua escolha em 1. Como normal, você não pode aumentar um valor de atributo acima de 20 usando essa característica.",
                            IdClasse = 1,
                            Nivel = 19,
                            Nome = "Incremento no Valor de Habilidade"
                        },
                        new
                        {
                            Id = 17,
                            Descricao = "No 20º nível, você pode extrair de sua reserva interna de poder místico enquanto roga ao seu patrono para recuperar espaços de magia gastos. Você pode gastar 1 minuto implorando pela ajuda do seu patrono para recuperar todos os seus espaços de magia gastos da sua característica Magia de Pacto. Você deve terminar um descanso longo antes de usar esta característica novamente.",
                            IdClasse = 1,
                            Nivel = 20,
                            Nome = "Mestre Místico"
                        },
                        new
                        {
                            Id = 18,
                            Descricao = "Com uma ação, você pode desaparecer magicamente e entrar em seu receptáculo, que permanece no espaço que você deixou. O interior do receptáculo é um espaço extradimensional na forma de um cilindro de 6m de raio e 6m de altura, e lembra o objeto. O interior é mobiliado com almofadas e mesas baixas e fica em uma temperatura confortável. Enquanto dentro, você pode ouvir a área ao redor do receptáculo como se estivesse em seu espaço. Você pode permanecer dentro do receptáculo por uma quantia de horas igual a duas vezes seu bônus de proficiência. Você pode sair do receptáculo antes desse tempo se usar uma ação bônus para isso, se você morrer, ou se o receptáculo for destruído. Quando você sai desse receptáculo, você aparece no espaço desocupado mais próximo dele. Qualquer objeto deixado no receptáculo permanece lá até ser carregado para fora, e se o receptáculo for destruído, todo objeto contido por ele reaparece sem danos nos espaços desocupados mais próximos à antiga localização do receptáculo. Uma vez que entre no receptáculo, você não pode fazer isso novamente até terminar um descanso longo.\n\nA CA do receptáculo é igual à sua CD de magia. Ele tem pontos de vida igual ao seu nível de bruxo mais seu bônus de proficiência, e ele é imune a dano venenoso e psíquico.\nSe o receptáculo for destruído, você pode realizar uma cerimônia de 1 hora para receber um substituto de seu patrono. Essa cerimônia pode ser realizada durante um descanso curto ou longo, e o receptáculo anterior é destruído caso ainda exista. O receptáculo desaparece em um clarão de poder elemental quando você morre.",
                            IdClasse = 3,
                            Nivel = 1,
                            Nome = "Descanso Engarrafado"
                        },
                        new
                        {
                            Id = 19,
                            Descricao = "Uma vez por turno, quando você acerta uma rolagem de ataque, você pode causar dano extra ao alvo igual ao seu bônus de proficiência. O tipo desse dano é determinado por seu patrono: contundente (dao), trovejante (djinni), ígneo (ifriti), ou gélido (marid).",
                            IdClasse = 3,
                            Nivel = 1,
                            Nome = "Ira do Gênio"
                        });
                });

            modelBuilder.Entity("Domain.Entities.Classe", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("DadoVida")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("character varying(10)");

                    b.Property<int?>("Deslocamento")
                        .HasColumnType("integer");

                    b.Property<int?>("IdClassePai")
                        .HasColumnType("integer");

                    b.Property<string>("Nome")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<int>("QtdPericiasEscolha")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("IdClassePai");

                    b.ToTable("Classes", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1,
                            DadoVida = "d8",
                            Nome = "Bruxo",
                            QtdPericiasEscolha = 2
                        },
                        new
                        {
                            Id = 2,
                            DadoVida = "d8",
                            Nome = "Monge",
                            QtdPericiasEscolha = 2
                        },
                        new
                        {
                            Id = 3,
                            DadoVida = "",
                            IdClassePai = 1,
                            Nome = "o Gênio",
                            QtdPericiasEscolha = 0
                        },
                        new
                        {
                            Id = 4,
                            DadoVida = "",
                            IdClassePai = 2,
                            Nome = "Caminho do eu astral",
                            QtdPericiasEscolha = 0
                        });
                });

            modelBuilder.Entity("Domain.Entities.ClassePericiaDisponivel", b =>
                {
                    b.Property<int>("IdClasse")
                        .HasColumnType("integer");

                    b.Property<int>("IdPericia")
                        .HasColumnType("integer");

                    b.HasKey("IdClasse", "IdPericia");

                    b.HasIndex("IdPericia");

                    b.ToTable("ClassesPericiasDisponiveis", (string)null);

                    b.HasData(
                        new
                        {
                            IdClasse = 1,
                            IdPericia = 3
                        },
                        new
                        {
                            IdClasse = 1,
                            IdPericia = 6
                        },
                        new
                        {
                            IdClasse = 1,
                            IdPericia = 8
                        },
                        new
                        {
                            IdClasse = 1,
                            IdPericia = 9
                        },
                        new
                        {
                            IdClasse = 1,
                            IdPericia = 11
                        },
                        new
                        {
                            IdClasse = 1,
                            IdPericia = 13
                        },
                        new
                        {
                            IdClasse = 1,
                            IdPericia = 17
                        });
                });

            modelBuilder.Entity("Domain.Entities.ClassePersonagem", b =>
                {
                    b.Property<int>("IdPersonagem")
                        .HasColumnType("integer");

                    b.Property<int>("IdClasse")
                        .HasColumnType("integer");

                    b.Property<int?>("IdSubclasse")
                        .HasColumnType("integer");

                    b.Property<int>("Nivel")
                        .HasColumnType("integer");

                    b.Property<string>("SubclasseEscolha")
                        .HasColumnType("text");

                    b.HasKey("IdPersonagem", "IdClasse");

                    b.HasIndex("IdClasse");

                    b.HasIndex("IdSubclasse");

                    b.ToTable("ClassesPersonagens", (string)null);

                    b.HasData(
                        new
                        {
                            IdPersonagem = 1,
                            IdClasse = 1,
                            IdSubclasse = 3,
                            Nivel = 1,
                            SubclasseEscolha = "Djinni"
                        });
                });

            modelBuilder.Entity("Domain.Entities.ClasseProgressao", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<int>("BonusProficiencia")
                        .HasColumnType("integer");

                    b.Property<int>("EspacosMagia")
                        .HasColumnType("integer");

                    b.Property<int>("IdClasse")
                        .HasColumnType("integer");

                    b.Property<int>("InvocacoesConhecidas")
                        .HasColumnType("integer");

                    b.Property<int>("MagiasConhecidas")
                        .HasColumnType("integer");

                    b.Property<int>("Nivel")
                        .HasColumnType("integer");

                    b.Property<int>("NivelMagia")
                        .HasColumnType("integer");

                    b.Property<int>("TruquesConhecidos")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("IdClasse");

                    b.ToTable("ClasseProgressoes", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1,
                            BonusProficiencia = 2,
                            EspacosMagia = 1,
                            IdClasse = 1,
                            InvocacoesConhecidas = 0,
                            MagiasConhecidas = 2,
                            Nivel = 1,
                            NivelMagia = 1,
                            TruquesConhecidos = 2
                        },
                        new
                        {
                            Id = 2,
                            BonusProficiencia = 2,
                            EspacosMagia = 2,
                            IdClasse = 1,
                            InvocacoesConhecidas = 2,
                            MagiasConhecidas = 3,
                            Nivel = 2,
                            NivelMagia = 1,
                            TruquesConhecidos = 2
                        },
                        new
                        {
                            Id = 3,
                            BonusProficiencia = 2,
                            EspacosMagia = 2,
                            IdClasse = 1,
                            InvocacoesConhecidas = 2,
                            MagiasConhecidas = 4,
                            Nivel = 3,
                            NivelMagia = 2,
                            TruquesConhecidos = 2
                        },
                        new
                        {
                            Id = 4,
                            BonusProficiencia = 2,
                            EspacosMagia = 2,
                            IdClasse = 1,
                            InvocacoesConhecidas = 3,
                            MagiasConhecidas = 5,
                            Nivel = 4,
                            NivelMagia = 2,
                            TruquesConhecidos = 3
                        },
                        new
                        {
                            Id = 5,
                            BonusProficiencia = 3,
                            EspacosMagia = 2,
                            IdClasse = 1,
                            InvocacoesConhecidas = 3,
                            MagiasConhecidas = 6,
                            Nivel = 5,
                            NivelMagia = 3,
                            TruquesConhecidos = 3
                        },
                        new
                        {
                            Id = 6,
                            BonusProficiencia = 3,
                            EspacosMagia = 2,
                            IdClasse = 1,
                            InvocacoesConhecidas = 4,
                            MagiasConhecidas = 7,
                            Nivel = 6,
                            NivelMagia = 3,
                            TruquesConhecidos = 3
                        },
                        new
                        {
                            Id = 7,
                            BonusProficiencia = 3,
                            EspacosMagia = 2,
                            IdClasse = 1,
                            InvocacoesConhecidas = 4,
                            MagiasConhecidas = 8,
                            Nivel = 7,
                            NivelMagia = 4,
                            TruquesConhecidos = 3
                        },
                        new
                        {
                            Id = 8,
                            BonusProficiencia = 3,
                            EspacosMagia = 2,
                            IdClasse = 1,
                            InvocacoesConhecidas = 4,
                            MagiasConhecidas = 9,
                            Nivel = 8,
                            NivelMagia = 4,
                            TruquesConhecidos = 3
                        },
                        new
                        {
                            Id = 9,
                            BonusProficiencia = 4,
                            EspacosMagia = 2,
                            IdClasse = 1,
                            InvocacoesConhecidas = 5,
                            MagiasConhecidas = 10,
                            Nivel = 9,
                            NivelMagia = 5,
                            TruquesConhecidos = 3
                        },
                        new
                        {
                            Id = 10,
                            BonusProficiencia = 4,
                            EspacosMagia = 2,
                            IdClasse = 1,
                            InvocacoesConhecidas = 5,
                            MagiasConhecidas = 10,
                            Nivel = 10,
                            NivelMagia = 5,
                            TruquesConhecidos = 4
                        },
                        new
                        {
                            Id = 11,
                            BonusProficiencia = 4,
                            EspacosMagia = 3,
                            IdClasse = 1,
                            InvocacoesConhecidas = 5,
                            MagiasConhecidas = 11,
                            Nivel = 11,
                            NivelMagia = 5,
                            TruquesConhecidos = 4
                        },
                        new
                        {
                            Id = 12,
                            BonusProficiencia = 4,
                            EspacosMagia = 3,
                            IdClasse = 1,
                            InvocacoesConhecidas = 6,
                            MagiasConhecidas = 11,
                            Nivel = 12,
                            NivelMagia = 5,
                            TruquesConhecidos = 4
                        },
                        new
                        {
                            Id = 13,
                            BonusProficiencia = 5,
                            EspacosMagia = 3,
                            IdClasse = 1,
                            InvocacoesConhecidas = 6,
                            MagiasConhecidas = 12,
                            Nivel = 13,
                            NivelMagia = 5,
                            TruquesConhecidos = 4
                        },
                        new
                        {
                            Id = 14,
                            BonusProficiencia = 5,
                            EspacosMagia = 3,
                            IdClasse = 1,
                            InvocacoesConhecidas = 6,
                            MagiasConhecidas = 12,
                            Nivel = 14,
                            NivelMagia = 5,
                            TruquesConhecidos = 4
                        },
                        new
                        {
                            Id = 15,
                            BonusProficiencia = 5,
                            EspacosMagia = 3,
                            IdClasse = 1,
                            InvocacoesConhecidas = 7,
                            MagiasConhecidas = 13,
                            Nivel = 15,
                            NivelMagia = 5,
                            TruquesConhecidos = 4
                        },
                        new
                        {
                            Id = 16,
                            BonusProficiencia = 5,
                            EspacosMagia = 3,
                            IdClasse = 1,
                            InvocacoesConhecidas = 7,
                            MagiasConhecidas = 13,
                            Nivel = 16,
                            NivelMagia = 5,
                            TruquesConhecidos = 4
                        },
                        new
                        {
                            Id = 17,
                            BonusProficiencia = 6,
                            EspacosMagia = 4,
                            IdClasse = 1,
                            InvocacoesConhecidas = 7,
                            MagiasConhecidas = 14,
                            Nivel = 17,
                            NivelMagia = 5,
                            TruquesConhecidos = 4
                        },
                        new
                        {
                            Id = 18,
                            BonusProficiencia = 6,
                            EspacosMagia = 4,
                            IdClasse = 1,
                            InvocacoesConhecidas = 8,
                            MagiasConhecidas = 14,
                            Nivel = 18,
                            NivelMagia = 5,
                            TruquesConhecidos = 4
                        },
                        new
                        {
                            Id = 19,
                            BonusProficiencia = 6,
                            EspacosMagia = 4,
                            IdClasse = 1,
                            InvocacoesConhecidas = 8,
                            MagiasConhecidas = 15,
                            Nivel = 19,
                            NivelMagia = 5,
                            TruquesConhecidos = 4
                        },
                        new
                        {
                            Id = 20,
                            BonusProficiencia = 6,
                            EspacosMagia = 4,
                            IdClasse = 1,
                            InvocacoesConhecidas = 8,
                            MagiasConhecidas = 15,
                            Nivel = 20,
                            NivelMagia = 5,
                            TruquesConhecidos = 4
                        });
                });

            modelBuilder.Entity("Domain.Entities.Equipamento", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<int?>("ClasseArmadura")
                        .HasColumnType("integer");

                    b.Property<string>("Dano")
                        .HasColumnType("text");

                    b.Property<string>("Descricao")
                        .HasColumnType("text");

                    b.Property<bool?>("DesvantagemFurtividade")
                        .HasColumnType("boolean");

                    b.Property<int?>("ForcaRequerida")
                        .HasColumnType("integer");

                    b.Property<int?>("ModificadorClasseArmadura")
                        .HasColumnType("integer");

                    b.Property<string>("Nome")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("character varying(150)");

                    b.Property<bool?>("PermiteDestreza")
                        .HasColumnType("boolean");

                    b.Property<double?>("Peso")
                        .HasColumnType("double precision");

                    b.Property<string>("Preco")
                        .HasColumnType("text");

                    b.Property<string>("ProficienciaRequerida")
                        .HasColumnType("text");

                    b.PrimitiveCollection<List<string>>("Propriedades")
                        .IsRequired()
                        .HasColumnType("text[]");

                    b.Property<string>("TipoDano")
                        .HasColumnType("text");

                    b.Property<string>("TipoEquipamento")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.HasKey("Id");

                    b.ToTable("Equipamentos", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Descricao = "Um belo alaúde de madeira polida, usado por bardos e músicos.",
                            Nome = "Alaúde",
                            Peso = 1.0,
                            Preco = "35 PO",
                            Propriedades = new List<string>(),
                            TipoEquipamento = "Outro"
                        },
                        new
                        {
                            Id = 2,
                            Descricao = "Uma bolsa de couro impermeável com todos os componentes materiais necessários para conjurar suas magias.",
                            Nome = "Bolsa de componentes",
                            Peso = 1.0,
                            Preco = "25 PO",
                            Propriedades = new List<string>(),
                            TipoEquipamento = "Outro"
                        },
                        new
                        {
                            Id = 3,
                            Dano = "1d4",
                            Nome = "Adaga",
                            Peso = 0.5,
                            Preco = "2 PO",
                            ProficienciaRequerida = "Armas simples",
                            Propriedades = new List<string> { "Acuidade", "Leve", "Arremesso" },
                            TipoDano = "perfurante",
                            TipoEquipamento = "Arma"
                        },
                        new
                        {
                            Id = 4,
                            Dano = "1d8",
                            Nome = "Clava Grande",
                            Peso = 5.0,
                            Preco = "2 PP",
                            ProficienciaRequerida = "Armas simples",
                            Propriedades = new List<string> { "Duas mãos", "Pesada" },
                            TipoDano = "concussão",
                            TipoEquipamento = "Arma"
                        },
                        new
                        {
                            Id = 5,
                            Descricao = "Um escudo de metal gravado com o brasão do reino, adiciona +2 à Classe de Armadura (CA).",
                            ModificadorClasseArmadura = 2,
                            Nome = "Escudo",
                            Peso = 3.0,
                            Preco = "10 PO",
                            ProficienciaRequerida = "Escudos",
                            Propriedades = new List<string>(),
                            TipoEquipamento = "Escudo"
                        },
                        new
                        {
                            Id = 6,
                            ClasseArmadura = 12,
                            Descricao = "Uma armadura feita de couro resistente, reforçado com rebites de metal. Fornece CA 12 + Modificador de Destreza.",
                            DesvantagemFurtividade = false,
                            ForcaRequerida = 0,
                            Nome = "Armadura de Couro Batido",
                            PermiteDestreza = true,
                            Peso = 6.5,
                            Preco = "45 PO",
                            ProficienciaRequerida = "Armaduras leves",
                            Propriedades = new List<string>(),
                            TipoEquipamento = "Armadura"
                        },
                        new
                        {
                            Id = 7,
                            Descricao = "Um conjunto de roupas robustas composto por botas, calças ou saia de lã, um cinto, uma túnica quente e um manto com capuz.",
                            Nome = "Roupas de viajante",
                            Peso = 1.8,
                            Preco = "2 PO",
                            Propriedades = new List<string>(),
                            TipoEquipamento = "Outro"
                        });
                });

            modelBuilder.Entity("Domain.Entities.HistoriaPersonagem", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<int>("Capitulo")
                        .HasColumnType("integer");

                    b.Property<int>("IdPersonagem")
                        .HasColumnType("integer");

                    b.Property<string>("Texto")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("TituloCapitulo")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.HasKey("Id");

                    b.HasIndex("IdPersonagem");

                    b.ToTable("HistoriasPersonagens", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Capitulo = 1,
                            IdPersonagem = 1,
                            Texto = "Em Feywild, as leis não são escritas em papel, são moldadas em magia e tradição. A Corte de Verão, guiada pela paixão inflamável, e a Corte de Inverno, regida pela frieza calculista, mantinham um distanciamento hostil há milênios. Mas a mãe de Nailo, uma nobre cavaleira do Verão, e seu pai, um estrategista do Inverno, cometeram o maior dos crimes: eles se apaixonaram.\n\nA união deles foi um segredo mantido a duras penas, até o dia em que Nailo nasceu. O choro da criança ecoou pela floresta mágica, e sua pele irradiava uma energia que oscilava entre o calor reconfortante e a brisa gélida. Ele era a prova viva de que a separação das cortes era uma mentira política. Quando os espiões feéricos descobriram o bebê, a notícia correu como fogo seco. A criança foi apelidada de \"O Eclipse\" — uma anomalia que ameaçava o status quo. Ambas as rainhas exigiram que a \"mancha\" fosse apagada. Sem exércitos ou aliados, os pais de Nailo abandonaram seus postos, seus títulos e fugiram para os ermos sombrios da Agrestia das Fadas.",
                            TituloCapitulo = "O Pecado do Eclipse"
                        },
                        new
                        {
                            Id = 2,
                            Capitulo = 2,
                            IdPersonagem = 1,
                            Texto = "Durante dez anos, a vida de Nailo foi uma eterna mudança. Ele não conhecia grandes castelos de cristal ou os bailes eternos do Verão. Seu mundo eram acampamentos escondidos, cavernas camufladas e o amor incondicional de pais que viviam com os ouvidos atentos a qualquer galho quebrado.\n\nApesar da vida dura e do medo constante, Nailo encontrou seu refúgio na Primavera. Seus pais faziam de tudo para blindá-lo do terror da perseguição, ensinando-o a ver a beleza nas pequenas coisas e a tirar alegria de qualquer situação. Ele aprendeu a se mover em silêncio absoluto observando a postura defensiva do pai e herdou o sorriso invencível da mãe. Em seus momentos de distração, quando se sentia seguro o suficiente para rir solto, seus olhos e a magia ao seu redor brilhavam em um tom de amarelo extremamente vivo e chamativo que contrastava com a escuridão da floresta em que se escondiam.",
                            TituloCapitulo = "A Infância nas Sombras"
                        },
                        new
                        {
                            Id = 3,
                            Capitulo = 3,
                            IdPersonagem = 1,
                            Texto = "O fim da paz provisória chegou na noite do décimo aniversário de Nailo. Os cães de caça da Corte de Inverno e os batedores implacáveis do Verão os encurralaram simultaneamente em um desfiladeiro. Sem saída, seus pais sacaram as armas, trocando um último olhar de cumplicidade. A ordem para o filho foi clara: Corra e não olhe para trás.\n\nNailo correu desesperado, com o som do combate e dos feitiços explodindo às suas costas. Em seu pânico, o chão cedeu. Ele rolou por uma encosta de terra escura e caiu dentro de uma fenda profunda, aterrissando no chão empoeirado de ruínas antigas e esquecidas até mesmo pelas fadas. O som das garras dos cães se aproximava rapidamente do buraco acima dele.\n\nTateando no escuro, sua mão esbarrou em algo: um pergaminho antigo (scroll) coberto por hieróglifos incompreensíveis. Preso a ele, havia um pequeno pedaço de papel desgastado com três palavras soltas em élfico antigo. Com as criaturas rosnando e começando a descer pela fenda, Nailo, tremendo, leu as palavras em voz alta. Ao acertar a pronúncia da última sílaba mágica, o pergaminho se despedaçou em chispas de luz, e o mundo ao redor do menino foi engolido pelo vazio.",
                            TituloCapitulo = "A Fenda Antiga"
                        },
                        new
                        {
                            Id = 4,
                            Capitulo = 4,
                            IdPersonagem = 1,
                            Texto = "O silêncio substituiu os rosnados. Nailo abriu os olhos e se viu em uma pequena sala de pedra fria. O chão estava coberto de símbolos arcanos desenhados a giz, papéis antigos voavam levemente e uma corrente colossal prendia a figura central da sala. No meio de um círculo ritualístico, encontrava-se um Djinni azul gigantesco. Mesmo sentado, a criatura era monumental — se Nailo ficasse de pé ao seu lado, mal bateria na altura do peito do ser elemental. Mais ao canto, jogada no chão como lixo, repousava uma adaga.\n\nA criança da Primavera, assustada com o que havia acontecido a segundos atrás, e confusa com o que estava acontecendo agora, se apresentou. Limpando a poeira das calças, o Djinni, surpreso, quebrou o silêncio.\n— A companhia não me desagrada — retumbou a voz profunda da entidade.\n\nDurante a conversa, o Djinni tentou explicar o nó mágico de seu tormento. Um feiticeiro antigo o havia selado dentro do semiplano contido naquela adaga, mas cometeu o requinte de crueldade de jogar a própria adaga para dentro da prisão. Um paradoxo mágico perfeito. Era impossível que a magia fosse quebrada de dentro para fora com os recursos que tinham ali, e na situação que eles se encontravam.\n\n— Mas você não faz parte dessa equação, criança — explicou o Gênio, apontando para uma porta negra no fundo da sala. — Você não está selado. Pode simplesmente passar por aquela porta e ir embora.\n\nNailo olhou para a porta, depois para as correntes pesadas, e caminhou até o gigante azul. Ignorando as regras complexas da magia arcana, he esticou a mãozinha e levantou o dedo mindinho em direção ao peito do Djinni.\n\n— Eu prometo de dedinho que mesmo que eu saia, eu irei voltar para te tirar daqui. Ninguém merece ficar aprisionado assim sem motivo.\n\nO Gênio arregalou os olhos. Em Feywild, uma promessa infantil carregava um peso cósmico imensurável. O conceito de confiança e amizade havia se perdido há séculos para o elemental, mas ali, diante de uma criança sorridente, o pacto arcano mais improvável da história foi forjado através de um mindinho estendido.\n\nO Djinni riu, uma risada rouca e genuína que fez a sala tremer, e aceitou a promessa, infundindo a primeira faísca de poder arcano na alma do garoto.\n\nNailo pegou a adaga do chão, caminhou até a porta negra e abriu a maçaneta. Antes de atravessar o umbral que o levaria para longe do plano da adaga, he olhou por cima do ombro, sorriu largo e disse:\n\n— Até mais.",
                            TituloCapitulo = "A Promessa de Dedinho"
                        },
                        new
                        {
                            Id = 5,
                            Capitulo = 5,
                            IdPersonagem = 1,
                            Texto = "Assim que a porta negra bateu às suas costas, Kairo se viu no meio de uma planície vasta, espremida entre as encostas de uma grande montanha e a floresta densa e mística de Mythos. Foi só quando o silêncio da noite o abraçou que o garoto arregalou os olhos: em meio a toda a confusão e promessas, ele havia esquecido de perguntar o nome da entidade milenar que acabou de salvar sua vida. A partir daquele dia, o ser todo-poderoso seria chamado apenas de \"Gênio\", \"Amigão\" ou \"Azulão\".\n\nOs primeiros anos na floresta foram de sobrevivência crua. Sem a proteção dos pais, Kairo dependia dos estrondos mágicos de seu Raio Místico — canalizados pela adaga — para abater animais. O trauma da perda cobrou seu preço. Durante muito tempo, as noites de transe feérico de Kairo eram conturbadas, e he acordava flutuando violentamente entre as estações: dias de Inverno profundo, onde a tristeza gélida não o deixava sair da caverna, seguidos de dias de Verão, onde a fúria flamejante o fazia destruir árvores com magia.\n\nPorém, a resiliência falou mais alto. Kairo percebeu que ceder à dor seria esquecer o que seus pais haviam se sacrificado para proteger. Após meses meditando, ele conseguiu ancorar sua alma. Decidiu que enfrentaria o mundo com o mesmo sorriso invencível de sua mãe, mas também com o olhar calculista de seu pai, estabilizando-se no centro em sua forma de Primavera.",
                            TituloCapitulo = "O \"Azulão\" e a Floresta"
                        },
                        new
                        {
                            Id = 6,
                            Capitulo = 6,
                            IdPersonagem = 1,
                            Texto = "A floresta, embora familiar, tornou-se solitária demais para um Eladrin da Primavera. As luzes de Ozenes, uma cidade portuária ao longe, o atraíram. Antes de se aventurar pelas ruas, Kairo passou dias escondido nos telhados, observando a dinâmica humana. Ele notou que as pessoas entregavam pequenos discos dourados umas às outras em troca de peixe e pão. Confuso, he entrou no semiplano da adaga e perguntou ao Azulão. O Gênio, com um suspiro cansado, explicou o conceito civilizado de \"dinheiro\".\n\nCom a lição anotada, Kairo traçou um plano. Ele entrou em transe, forçando sua mente a assumir a frieza do Inverno. Na manhã seguinte, com cabelos lisos, brancos e pele pálida, o garoto sorrateiro roubou duas moedas de ouro de mercadores desatentos, escondendo-se antes do pôr do sol. No outro dia, ele acordou em sua radiante forma de Primavera: cabelos verdes esvoaçantes e pele brilhante. Kairo entrou na taverna local e, achando que duas moedas de ouro eram o equivalente a uma fortuna inesgotável, pediu tudo o que tinha direito. Ele devorou porções infinitas de peixe fatiado e bolinhos de arroz.\n\nQuando a conta chegou, o choque: as moedas não cobriam o banquete. Para não ser jogado no mar, o jovem precisou lavar pratos e limpar mesas por semanas. O castigo, no entanto, foi sua maior escola. Trabalhando ali, Kairo aprendeu a ler as pessoas, a ouvir boatos e a entender como Ozenes funcionava. Logo, ele passou a fazer pequenos bicos carregando cargas para os mercadores no cais, intercalando com dias ocasionais de \"Inverno\" para realizar furtos precisos quando o dinheiro faltava. Um ladrão de cabelos brancos roubava à noite, e um trabalhador sorridente de cabelos verdes gastava o dinheiro de dia.",
                            TituloCapitulo = "O Valor do Ouro e a Máscara de Inverno"
                        },
                        new
                        {
                            Id = 7,
                            Capitulo = 7,
                            IdPersonagem = 1,
                            Texto = "Aos poucos, carregar caixotes perdeu a graça, e Kairo se registrou na Guilda local para missões de escolta e proteção nas estradas. Foi assim que ele conheceu seu maior obstáculo: os Goblins. Passando semanas emboscado nas matas ao redor da cidade e observando os acampamentos dessas criaturas, a inteligência de Kairo brilhou. Sem professor algum, apenas ouvindo e deduzindo, ele aprendeu o idioma Goblin na marra, o que o tornou um aventureiro muito mais eficiente.\n\nO Gênio assistia a tudo isso de dentro da lâmina. A entidade milenar, que a princípio via Kairo apenas como um bilhete de saída aleatório, começou a perceber o verdadeiro potencial do garoto. Ele levava jeito para a coisa. Contudo, o Azulão também notou algo que o sorriso da Primavera tentava esconder: nos momentos de silêncio, a solidão de Kairo era ensurdecedora.\n\nEm uma de suas visitas à adaga, o Gênio materializou um livro de couro escuro e o jogou no colo de Kairo.\n— Você tem sido útil, criança. Mas está canalizando magia como um amador — resmungou o Gênio, disfarçando qualquer traço de empatia com seu tom pragmático de sempre. — Use isso. Tem algumas anotações que podem evitar que você morra de forma estúpida e me deixe preso aqui para sempre.\n\nAquele era o seu Livro das Sombras. Ao folhear as páginas rúnicas, Kairo encontrou feitiços novos, mas um em especial chamou sua atenção: Encontrar Familiar. Naquela mesma noite, em seu quarto alugado em Ozenes, Kairo conjurou a magia. Das sombras e da luz feérica, surgiu Shift. O pequeno companheiro animal — que mudava de forma dependendo do dia, ora um lagarto sorrateiro, ora um falcão ágil — tornou-se seu companheiro.\n\nAgora, aos 20 anos, armado com seu sorriso extravagante, seu fiel familiar Shift, a magia trovejante de um Gênio que virou um amigo rabugento e uma promessa de dedinho que ecoava pelo multiverso, Kairo estava pronto para deixar Ozenes e abraçar seu destiny.",
                            TituloCapitulo = "Goblins, Solidão e o Tomo Arcano"
                        });
                });

            modelBuilder.Entity("Domain.Entities.Idioma", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("Nome")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.HasKey("Id");

                    b.ToTable("Idiomas", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Nome = "Comum"
                        },
                        new
                        {
                            Id = 2,
                            Nome = "Anão"
                        },
                        new
                        {
                            Id = 3,
                            Nome = "Élfico"
                        },
                        new
                        {
                            Id = 4,
                            Nome = "Gigante"
                        },
                        new
                        {
                            Id = 5,
                            Nome = "Gnomo"
                        },
                        new
                        {
                            Id = 6,
                            Nome = "Goblin"
                        },
                        new
                        {
                            Id = 7,
                            Nome = "Halfling"
                        },
                        new
                        {
                            Id = 8,
                            Nome = "Orc"
                        });
                });

            modelBuilder.Entity("Domain.Entities.Magia", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("Descricao")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<int>("Nivel")
                        .HasColumnType("integer");

                    b.Property<string>("Nome")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.HasKey("Id");

                    b.ToTable("Magias", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Descricao = "Um feixe de energia estalante vai em direção a uma criatura. Faça um ataque à distância com magia. Com um acerto, o alvo sofre 1d10 de dano de energia. **Explosão Agonizante:** Você adiciona seu Modificador de Carisma (+4) ao dano. Dano total por acerto: 1d10+4 de Energia.",
                            Nivel = 0,
                            Nome = "Raio místico"
                        });
                });

            modelBuilder.Entity("Domain.Entities.Pericia", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("ModificadorAtributo")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<string>("Nome")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.HasKey("Id");

                    b.ToTable("Pericias", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1,
                            ModificadorAtributo = "Destreza",
                            Nome = "Acrobacia"
                        },
                        new
                        {
                            Id = 2,
                            ModificadorAtributo = "Sabedoria",
                            Nome = "Adestrar Animais"
                        },
                        new
                        {
                            Id = 3,
                            ModificadorAtributo = "Inteligência",
                            Nome = "Arcana"
                        },
                        new
                        {
                            Id = 4,
                            ModificadorAtributo = "Força",
                            Nome = "Atletismo"
                        },
                        new
                        {
                            Id = 5,
                            ModificadorAtributo = "Carisma",
                            Nome = "Atuação"
                        },
                        new
                        {
                            Id = 6,
                            ModificadorAtributo = "Carisma",
                            Nome = "Enganação"
                        },
                        new
                        {
                            Id = 7,
                            ModificadorAtributo = "Destreza",
                            Nome = "Furtividade"
                        },
                        new
                        {
                            Id = 8,
                            ModificadorAtributo = "Inteligência",
                            Nome = "História"
                        },
                        new
                        {
                            Id = 9,
                            ModificadorAtributo = "Carisma",
                            Nome = "Intimidação"
                        },
                        new
                        {
                            Id = 10,
                            ModificadorAtributo = "Sabedoria",
                            Nome = "Intuição"
                        },
                        new
                        {
                            Id = 11,
                            ModificadorAtributo = "Inteligência",
                            Nome = "Investigação"
                        },
                        new
                        {
                            Id = 12,
                            ModificadorAtributo = "Sabedoria",
                            Nome = "Medicina"
                        },
                        new
                        {
                            Id = 13,
                            ModificadorAtributo = "Inteligência",
                            Nome = "Natureza"
                        },
                        new
                        {
                            Id = 14,
                            ModificadorAtributo = "Sabedoria",
                            Nome = "Percepção"
                        },
                        new
                        {
                            Id = 15,
                            ModificadorAtributo = "Carisma",
                            Nome = "Persuasão"
                        },
                        new
                        {
                            Id = 16,
                            ModificadorAtributo = "Destreza",
                            Nome = "Prestidigitação"
                        },
                        new
                        {
                            Id = 17,
                            ModificadorAtributo = "Inteligência",
                            Nome = "Religião"
                        },
                        new
                        {
                            Id = 18,
                            ModificadorAtributo = "Sabedoria",
                            Nome = "Sobrevivência"
                        });
                });

            modelBuilder.Entity("Domain.Entities.Personagem", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("Alinhamento")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<string>("Base64Imagem")
                        .HasColumnType("text");

                    b.Property<int>("Carisma")
                        .HasColumnType("integer");

                    b.Property<string>("Codigo")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<int>("Constituicao")
                        .HasColumnType("integer");

                    b.Property<int>("Destreza")
                        .HasColumnType("integer");

                    b.Property<int>("Forca")
                        .HasColumnType("integer");

                    b.Property<int>("IdRaca")
                        .HasColumnType("integer");

                    b.Property<int>("Inteligencia")
                        .HasColumnType("integer");

                    b.Property<string>("Nome")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<int>("PecaCobre")
                        .HasColumnType("integer");

                    b.Property<int>("PecaElectro")
                        .HasColumnType("integer");

                    b.Property<int>("PecaOuro")
                        .HasColumnType("integer");

                    b.Property<int>("PecaPlatina")
                        .HasColumnType("integer");

                    b.Property<int>("PecaPrata")
                        .HasColumnType("integer");

                    b.Property<int>("Sabedoria")
                        .HasColumnType("integer");

                    b.Property<int>("VidaAtual")
                        .HasColumnType("integer");

                    b.Property<int>("VidaMaxima")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("Codigo")
                        .IsUnique();

                    b.HasIndex("IdRaca");

                    b.ToTable("Personagens", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Alinhamento = "Caótico e Bom",
                            Carisma = 19,
                            Codigo = "kairo",
                            Constituicao = 13,
                            Destreza = 16,
                            Forca = 8,
                            IdRaca = 1,
                            Inteligencia = 13,
                            Nome = "Kairo",
                            PecaCobre = 0,
                            PecaElectro = 0,
                            PecaOuro = 57,
                            PecaPlatina = 0,
                            PecaPrata = 0,
                            Sabedoria = 12,
                            VidaAtual = 9,
                            VidaMaxima = 9
                        });
                });

            modelBuilder.Entity("Domain.Entities.PersonagemEquipamento", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<int>("IdEquipamento")
                        .HasColumnType("integer");

                    b.Property<int>("IdPersonagem")
                        .HasColumnType("integer");

                    b.Property<bool>("IsEquipado")
                        .HasColumnType("boolean");

                    b.HasKey("Id");

                    b.HasIndex("IdEquipamento");

                    b.HasIndex("IdPersonagem");

                    b.ToTable("PersonagensEquipamentos", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1,
                            IdEquipamento = 1,
                            IdPersonagem = 1,
                            IsEquipado = false
                        },
                        new
                        {
                            Id = 2,
                            IdEquipamento = 2,
                            IdPersonagem = 1,
                            IsEquipado = false
                        },
                        new
                        {
                            Id = 3,
                            IdEquipamento = 3,
                            IdPersonagem = 1,
                            IsEquipado = false
                        },
                        new
                        {
                            Id = 4,
                            IdEquipamento = 5,
                            IdPersonagem = 1,
                            IsEquipado = false
                        },
                        new
                        {
                            Id = 5,
                            IdEquipamento = 6,
                            IdPersonagem = 1,
                            IsEquipado = false
                        },
                        new
                        {
                            Id = 6,
                            IdEquipamento = 7,
                            IdPersonagem = 1,
                            IsEquipado = false
                        });
                });

            modelBuilder.Entity("Domain.Entities.PersonagemIdioma", b =>
                {
                    b.Property<int>("IdPersonagem")
                        .HasColumnType("integer");

                    b.Property<int>("IdIdioma")
                        .HasColumnType("integer");

                    b.HasKey("IdPersonagem", "IdIdioma");

                    b.HasIndex("IdIdioma");

                    b.ToTable("PersonagensIdiomas", (string)null);

                    b.HasData(
                        new
                        {
                            IdPersonagem = 1,
                            IdIdioma = 1
                        });
                });

            modelBuilder.Entity("Domain.Entities.PersonagemMagia", b =>
                {
                    b.Property<int>("IdPersonagem")
                        .HasColumnType("integer");

                    b.Property<int>("IdMagia")
                        .HasColumnType("integer");

                    b.HasKey("IdPersonagem", "IdMagia");

                    b.HasIndex("IdMagia");

                    b.ToTable("PersonagensMagias", (string)null);

                    b.HasData(
                        new
                        {
                            IdPersonagem = 1,
                            IdMagia = 1
                        });
                });

            modelBuilder.Entity("Domain.Entities.PersonagemPericia", b =>
                {
                    b.Property<int>("IdPersonagem")
                        .HasColumnType("integer");

                    b.Property<int>("IdPericia")
                        .HasColumnType("integer");

                    b.Property<bool>("IsMaestria")
                        .HasColumnType("boolean");

                    b.Property<bool>("IsProficiente")
                        .HasColumnType("boolean");

                    b.Property<string>("Origem")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("character varying(150)");

                    b.HasKey("IdPersonagem", "IdPericia");

                    b.HasIndex("IdPericia");

                    b.ToTable("PersonagensPericias", (string)null);

                    b.HasData(
                        new
                        {
                            IdPersonagem = 1,
                            IdPericia = 3,
                            IsMaestria = false,
                            IsProficiente = true,
                            Origem = "Classe (Bruxo)"
                        },
                        new
                        {
                            IdPersonagem = 1,
                            IdPericia = 6,
                            IsMaestria = false,
                            IsProficiente = true,
                            Origem = "Classe (Bruxo)"
                        },
                        new
                        {
                            IdPersonagem = 1,
                            IdPericia = 8,
                            IsMaestria = false,
                            IsProficiente = true,
                            Origem = "Antecedente (Charlatão)"
                        },
                        new
                        {
                            IdPersonagem = 1,
                            IdPericia = 7,
                            IsMaestria = false,
                            IsProficiente = true,
                            Origem = "Antecedente (Charlatão)"
                        });
                });

            modelBuilder.Entity("Domain.Entities.PersonagemProficiencia", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<int>("IdPersonagem")
                        .HasColumnType("integer");

                    b.Property<string>("Nome")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("character varying(150)");

                    b.Property<string>("Origem")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("character varying(150)");

                    b.Property<string>("Tipo")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.HasKey("Id");

                    b.HasIndex("IdPersonagem");

                    b.ToTable("PersonagensProficiencias", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1,
                            IdPersonagem = 1,
                            Nome = "Armas simples",
                            Origem = "Classe (Bruxo)",
                            Tipo = "Arma"
                        },
                        new
                        {
                            Id = 2,
                            IdPersonagem = 1,
                            Nome = "Armaduras leves",
                            Origem = "Classe (Bruxo)",
                            Tipo = "Armadura"
                        },
                        new
                        {
                            Id = 3,
                            IdPersonagem = 1,
                            Nome = "Nenhuma",
                            Origem = "Classe (Bruxo)",
                            Tipo = "Ferramenta"
                        });
                });

            modelBuilder.Entity("Domain.Entities.PersonagemSalvaguarda", b =>
                {
                    b.Property<int>("IdPersonagem")
                        .HasColumnType("integer");

                    b.Property<string>("Atributo")
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<bool>("IsProficiente")
                        .HasColumnType("boolean");

                    b.HasKey("IdPersonagem", "Atributo");

                    b.ToTable("PersonagensSalvaguardas", (string)null);

                    b.HasData(
                        new
                        {
                            IdPersonagem = 1,
                            Atributo = "Sabedoria",
                            IsProficiente = true
                        },
                        new
                        {
                            IdPersonagem = 1,
                            Atributo = "Carisma",
                            IsProficiente = true
                        });
                });

            modelBuilder.Entity("Domain.Entities.Raca", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("Descricao")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<int>("Deslocamento")
                        .HasColumnType("integer");

                    b.Property<string>("Nome")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<string>("Tamanho")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<string>("TipoCriatura")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("character varying(150)");

                    b.HasKey("Id");

                    b.ToTable("Racas", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Descricao = "Eladrin são elfos da Feywild, um reino de beleza perigosa e magia sem limites. Usando essa magia, eladrin pode ir de um lugar para outro em um piscar de olhos, e cada eladrin ressoa com emoções capturadas na feywild na forma de estações – afinidades que afetam o humor e a aparência do eladrin. A estação de um eladrin pode mudar, embora alguns permaneçam em uma estação para sempre. escolha sua estação ou role na tabela de estações eladrin. Seu traço de Transe permite que você mude sua estação. Como outros elfos, os eladrin podem viver mais de 750 anos.",
                            Deslocamento = 9,
                            Nome = "Eladrin",
                            Tamanho = "Médio",
                            TipoCriatura = "Humanóide. Você também é considerado um elfo para qualquer pré-requisito ou efeito que exija que você seja um elfo."
                        });
                });

            modelBuilder.Entity("Domain.Entities.TracoRacial", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("Descricao")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<int>("IdRaca")
                        .HasColumnType("integer");

                    b.Property<string>("Nome")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.HasKey("Id");

                    b.HasIndex("IdRaca");

                    b.ToTable("TracosRaciais", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Descricao = "Você pode ver na penumbra a até 18 metros(60ft) de você como se fosse luz brilhante, e na escuridão como se fosse luz fraca. Você não pode discernir cores na escuridão, apenas tons de cinza.",
                            IdRaca = 1,
                            Nome = "Visão no escuro"
                        },
                        new
                        {
                            Id = 2,
                            Descricao = "Você tem vantagem nos testes de resistência que fizer para evitar ou acabar com a condição de encantado em si mesmo.",
                            IdRaca = 1,
                            Nome = "Ancestralidade Feérica"
                        },
                        new
                        {
                            Id = 3,
                            Descricao = "Como ação bônus, você pode se teletransportar magicamente até 9 metros(30ft) para um espaço desocupado que você possa ver. Você pode usar esse traço um número de vezes igual ao seu bônus de proficiência, e você recupera todos os usos gastos quando terminar um descanso longo. Quando você alcança o 3° nível, seu Passo Feérico ganha um efeito adicional baseado em sua estação; se o efeito exigir um teste de resistência, a CD é igual a 8 + seu bônus de proficiência + seu modificador de Inteligência, Sabedoria ou Carisma (escolha ao selecionar esta raça):\n\nOutono. Imediatamente após você usar seu Passo Feérico, até duas criaturas de sua escolha que você possa ver a até 3 metros(10ft) de você devem ser bem sucedidas em um teste de resistência de Sabedoria ou serão enfeitiçadas por você por 1 minuto, ou até que você ou seus companheiros causem qualquer dano às criaturas.\n\nInverno. Quando você usa seu Passo Feérico, uma criatura de sua escolha que você possa ver a 1,5 metro(5ft) de você antes de se teletransportar deve fazer um teste de resistência de Sabedoria ou ficará com medo de você até o final do seu próximo turno.\n\nPrimavera. Quando você usa seu Passo Feérico, você pode tocar uma criatura voluntária a até 1,5 metros(5ft) de você. Essa criatura então se teletransporta em vez de você, aparecendo em um espaço desocupado de sua escolha que você pode ver a até 9 metros(30ft) de você.\n\nVerão. Imediatamente após você usar seu Passo Feérico, cada criatura de sua escolha que você possa ver a até 1,5 metros(5ft) de você sofre dano de fogo igual ao seu bônus de proficiência.",
                            IdRaca = 1,
                            Nome = "Passo Feérico"
                        },
                        new
                        {
                            Id = 4,
                            Descricao = "Você tem proficiência na perícia Percepção.",
                            IdRaca = 1,
                            Nome = "Sentidos Aguçados"
                        },
                        new
                        {
                            Id = 5,
                            Descricao = "Você não precisa dormir, e magia não pode fazer você dormir. Você pode terminar um descanso longo em 4 horas se passar essas horas in uma meditação como um transe, durante a qual você retém consciência. Sempre que você terminar este transe, você pode mudar sua temporada, e você pode ganhar duas proficiências que você não possui, cada uma com uma arma ou ferramenta de sua escolha selecionada no Livro do Jogador. Você adquire misticamente essas proficiências extraindo-as da memória élfica compartilhada e as mantém até terminar um descanso longo.",
                            IdRaca = 1,
                            Nome = "Transe"
                        });
                });

            modelBuilder.Entity("Domain.Entities.TracoRacialPericia", b =>
                {
                    b.Property<int>("IdTracoRacial")
                        .HasColumnType("integer");

                    b.Property<int>("IdPericia")
                        .HasColumnType("integer");

                    b.Property<bool>("IsMaestria")
                        .HasColumnType("boolean");

                    b.HasKey("IdTracoRacial", "IdPericia");

                    b.HasIndex("IdPericia");

                    b.ToTable("TracosRaciaisPericias", (string)null);

                    b.HasData(
                        new
                        {
                            IdTracoRacial = 4,
                            IdPericia = 14,
                            IsMaestria = false
                        });
                });

            modelBuilder.Entity("Domain.Entities.AcaoPersonagem", b =>
                {
                    b.HasOne("Domain.Entities.Personagem", "Personagem")
                        .WithMany("Acoes")
                        .HasForeignKey("IdPersonagem")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Personagem");
                });

            modelBuilder.Entity("Domain.Entities.CaracteristicaClasse", b =>
                {
                    b.HasOne("Domain.Entities.Classe", "Classe")
                        .WithMany("Caracteristicas")
                        .HasForeignKey("IdClasse")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Classe");
                });

            modelBuilder.Entity("Domain.Entities.Classe", b =>
                {
                    b.HasOne("Domain.Entities.Classe", "ClassePai")
                        .WithMany("Subclasses")
                        .HasForeignKey("IdClassePai")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("ClassePai");
                });

            modelBuilder.Entity("Domain.Entities.ClassePericiaDisponivel", b =>
                {
                    b.HasOne("Domain.Entities.Classe", "Classe")
                        .WithMany("PericiasDisponiveis")
                        .HasForeignKey("IdClasse")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Domain.Entities.Pericia", "Pericia")
                        .WithMany("ClassesPericiasDisponiveis")
                        .HasForeignKey("IdPericia")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Classe");

                    b.Navigation("Pericia");
                });

            modelBuilder.Entity("Domain.Entities.ClassePersonagem", b =>
                {
                    b.HasOne("Domain.Entities.Classe", "Classe")
                        .WithMany("ClassesPersonagens")
                        .HasForeignKey("IdClasse")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Domain.Entities.Personagem", "Personagem")
                        .WithMany("ClassesPersonagens")
                        .HasForeignKey("IdPersonagem")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Domain.Entities.Classe", "Subclasse")
                        .WithMany()
                        .HasForeignKey("IdSubclasse")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("Classe");

                    b.Navigation("Personagem");

                    b.Navigation("Subclasse");
                });

            modelBuilder.Entity("Domain.Entities.ClasseProgressao", b =>
                {
                    b.HasOne("Domain.Entities.Classe", "Classe")
                        .WithMany("Progressoes")
                        .HasForeignKey("IdClasse")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Classe");
                });

            modelBuilder.Entity("Domain.Entities.HistoriaPersonagem", b =>
                {
                    b.HasOne("Domain.Entities.Personagem", "Personagem")
                        .WithMany("Historias")
                        .HasForeignKey("IdPersonagem")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Personagem");
                });

            modelBuilder.Entity("Domain.Entities.Personagem", b =>
                {
                    b.HasOne("Domain.Entities.Raca", "Raca")
                        .WithMany("Personagens")
                        .HasForeignKey("IdRaca")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Raca");
                });

            modelBuilder.Entity("Domain.Entities.PersonagemEquipamento", b =>
                {
                    b.HasOne("Domain.Entities.Equipamento", "Equipamento")
                        .WithMany()
                        .HasForeignKey("IdEquipamento")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Domain.Entities.Personagem", "Personagem")
                        .WithMany("Equipamentos")
                        .HasForeignKey("IdPersonagem")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Equipamento");

                    b.Navigation("Personagem");
                });

            modelBuilder.Entity("Domain.Entities.PersonagemIdioma", b =>
                {
                    b.HasOne("Domain.Entities.Idioma", "Idioma")
                        .WithMany("PersonagensIdiomas")
                        .HasForeignKey("IdIdioma")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Domain.Entities.Personagem", "Personagem")
                        .WithMany("PersonagensIdiomas")
                        .HasForeignKey("IdPersonagem")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Idioma");

                    b.Navigation("Personagem");
                });

            modelBuilder.Entity("Domain.Entities.PersonagemMagia", b =>
                {
                    b.HasOne("Domain.Entities.Magia", "Magia")
                        .WithMany("PersonagensMagias")
                        .HasForeignKey("IdMagia")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Domain.Entities.Personagem", "Personagem")
                        .WithMany("PersonagensMagias")
                        .HasForeignKey("IdPersonagem")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Magia");

                    b.Navigation("Personagem");
                });

            modelBuilder.Entity("Domain.Entities.PersonagemPericia", b =>
                {
                    b.HasOne("Domain.Entities.Pericia", "Pericia")
                        .WithMany("PersonagensPericias")
                        .HasForeignKey("IdPericia")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Domain.Entities.Personagem", "Personagem")
                        .WithMany("PersonagensPericias")
                        .HasForeignKey("IdPersonagem")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Pericia");

                    b.Navigation("Personagem");
                });

            modelBuilder.Entity("Domain.Entities.PersonagemProficiencia", b =>
                {
                    b.HasOne("Domain.Entities.Personagem", "Personagem")
                        .WithMany("Proficiencias")
                        .HasForeignKey("IdPersonagem")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Personagem");
                });

            modelBuilder.Entity("Domain.Entities.PersonagemSalvaguarda", b =>
                {
                    b.HasOne("Domain.Entities.Personagem", "Personagem")
                        .WithMany("Salvaguardas")
                        .HasForeignKey("IdPersonagem")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Personagem");
                });

            modelBuilder.Entity("Domain.Entities.TracoRacial", b =>
                {
                    b.HasOne("Domain.Entities.Raca", "Raca")
                        .WithMany("TracosRaciais")
                        .HasForeignKey("IdRaca")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Raca");
                });

            modelBuilder.Entity("Domain.Entities.TracoRacialPericia", b =>
                {
                    b.HasOne("Domain.Entities.Pericia", "Pericia")
                        .WithMany("TracosRaciaisPericias")
                        .HasForeignKey("IdPericia")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Domain.Entities.TracoRacial", "TracoRacial")
                        .WithMany("TracosRaciaisPericias")
                        .HasForeignKey("IdTracoRacial")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Pericia");

                    b.Navigation("TracoRacial");
                });

            modelBuilder.Entity("Domain.Entities.Classe", b =>
                {
                    b.Navigation("Caracteristicas");

                    b.Navigation("ClassesPersonagens");

                    b.Navigation("PericiasDisponiveis");

                    b.Navigation("Progressoes");

                    b.Navigation("Subclasses");
                });

            modelBuilder.Entity("Domain.Entities.Idioma", b =>
                {
                    b.Navigation("PersonagensIdiomas");
                });

            modelBuilder.Entity("Domain.Entities.Magia", b =>
                {
                    b.Navigation("PersonagensMagias");
                });

            modelBuilder.Entity("Domain.Entities.Pericia", b =>
                {
                    b.Navigation("ClassesPericiasDisponiveis");

                    b.Navigation("PersonagensPericias");

                    b.Navigation("TracosRaciaisPericias");
                });

            modelBuilder.Entity("Domain.Entities.Personagem", b =>
                {
                    b.Navigation("Acoes");

                    b.Navigation("ClassesPersonagens");

                    b.Navigation("Equipamentos");

                    b.Navigation("Historias");

                    b.Navigation("PersonagensIdiomas");

                    b.Navigation("PersonagensMagias");

                    b.Navigation("PersonagensPericias");

                    b.Navigation("Proficiencias");

                    b.Navigation("Salvaguardas");
                });

            modelBuilder.Entity("Domain.Entities.Raca", b =>
                {
                    b.Navigation("Personagens");

                    b.Navigation("TracosRaciais");
                });

            modelBuilder.Entity("Domain.Entities.TracoRacial", b =>
                {
                    b.Navigation("TracosRaciaisPericias");
                });
#pragma warning restore 612, 618
        }
    }
}
